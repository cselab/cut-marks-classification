%invImgInd Search index that maps visual words to images.
%  imageIndex = invImgInd(bag) returns an invImgInd
%  object given a bof object, bag. Add images to the imageIndex
%  using the addImages method. Then use the retrieveImages function to
%  search for images within the imageIndex.
%
%  [...] = invImgInd(...,'SaveFeatureLocations', value) optionally
...
%     bof      - bof object used by the index.
...
% bag = bof(imds, 'PointSelection', 'Detector', 'VocabularySize', 1000);
%
% % Create an image search index and add images
% imageIndex = invImgInd(bag);
% 
...
classdef invImgInd < vision.internal.EnforceScalarHandle   
    ...
    properties(GetAccess = public, SetAccess = protected)                
        ...
        % bof - The bof object used in the index.
        bof               
    end
    ...
    methods
        ...
        % -----------------------------------------------------------------
        function this = invImgInd(bag, varargin)
            
            % Set parameter defaults.
            defaults = invImgInd.getParameterDefaults();
            
            this.MatchThreshold     = defaults.MatchThreshold;
            this.WordFrequencyRange = defaults.WordFrequencyRange;           
                        
            params = invImgInd.parseInputs(bag, varargin{:});
        ...
        function [imageIDs, varargout] = search(this, query, varargin)                                    
            ...
            params = invImgInd.parseSearchInputs(query,varargin{:});
            
            if this.SaveLocations
                [queryHist, queryVisualWords] = this.bof.encode(query,'Normalization','none','SparseOutput',true);
            else
                queryHist = this.bof.encode(query,'Normalization','none','SparseOutput',true);
        ...
        function addImages(this, imgSet, varargin)   
            ...
            params = invImgInd.parseAddImagesInputs(imgSet, varargin{:});
            
            if this.SaveLocations
                % Index word histograms and word location information
                [wordHistograms, words] = this.bof.encode(imgSet, ...                
                    'Normalization', 'None', ...
                    'SparseOutput',  true,...
                ...
                wordHistograms = this.bof.encode(imgSet, ...
                    'Normalization', 'None', ...
                    'SparseOutput',  true,...
                    'Verbose',       params.Verbose,...
                    'UseParallel',   params.UseParallel);
    ...
    methods(Access = protected, Hidden)
        ...
        function [words, queryWords] = removeStopWords(this, queryHist, queryWords, freqRange)
            ...
            if this.SaveLocations
                ...
                queryWords = vision.internal.visualWords(w,l,this.bof.VocabularySize);
            end
        end
        ...
        function scores = computeMatchMetric(this, imageIDs, words, queryFeatures)
            ...
            indexFeatures = invImgInd.l2NormalizeFeatures(indexFeatures);
            queryFeatures = invImgInd.l2NormalizeFeatures(queryFeatures);
    ...
    methods(Static, Access = private)       
        ...
        function params = parseSearchInputs(I, varargin)                        
            ...
            defaults = invImgInd.getSearchParameterDefaults();
        ...
        function params = parseInputs(bag, varargin)
            ...
            validateattributes(bag, {'bof'},...
                {'nonempty'},mfilename,'bag');
            
            p = inputParser();                                                                          
            
            d = invImgInd.getParameterDefaults();
