...
%   algorithm.
%
%   bag = bof(imds,'CustomExtractor',extractorFcn) returns a
...
%
%     <a href="matlab:helpview(fullfile(docroot,'toolbox','vision','vision.map'),'bofCustomFeatures')">Learn more about writing a custom feature extractor.</a>
%
%     <a href="matlab:bof.createExtractorTemplate">Open a custom feature extractor template.</a>
%
%   bag = bof(...,Name,Value) specifies additional
...
%   
%   - bof supports parallel computing using multiple MATLAB
...
%   bag = bof(trainingSet); % bag creation can take a few minutes
%
...
%   extractor = @examplebofExtractor;
%   bag = bof(imds,'CustomExtractor',extractor) 
%
%   % <a href="matlab:edit('examplebofExtractor')">Open examplebofExtractor for more details.</a>
%
...
classdef bof < vision.internal.EnforceScalarHandle & matlab.mixin.CustomDisplay
...
%         Vocabulary; 
    ...
    methods (Access = public)
        ...
        function [featureVector, varargout] = encode(this, in, varargin)
            ...
            params = bof.parseEncodeInputs(in, varargin{:});
            ...
            if ~isnumeric(in)        
                ...
                bof.printImageSetDescription(printer, in);

                if bof.isImageSet(in)
                    numImages = sum([in.Count]);
                    
                    featureVector = bof.allocateFeatureVector(numImages, this.VocabularySize, params.SparseOutput);
    ...
    methods (Hidden, Access = protected)
        ...
        %    
            ...
            bof.printImageSetDescription(printer,imgSets);
             
            this.printPointSelectionInfo(printer);
            
            if bof.isImageSet(imgSets)
        ...
        function [groupedDescriptors, groupedScores] = extractDescriptorsFromImageDS(this, imds, params)
            ...
            if params.UseParallel  
                parfor i = 1:numImages
                    ...
                    [descriptors{i}, scores{i}, ~, points{i}] = ...
                        this.Extractor(img); %#ok<PFBNS>
                end                
            else
                for i = 1:numImages
            ...
            numCategories      = bof.getNumCategories(imds);
        ...
        %------------------------------------------------------------------
        function [descriptors, scores, locations, points] = extractDescriptorsFromImage(this, image)
        ...
        %------------------------------------------------------------------
        function trimmedDescriptors = trimDescriptors(this, descriptorCell, scores, printer)
        ...
        %------------------------------------------------------------------
            ...
            features = bof.allocateFeatureVector(numImages, this.VocabularySize, params.SparseOutput); 
            words    = bof.allocateVisualWords(numImages);
        ...
        function clusterCenters = createVocabulary(this, descriptors, varargin)
            ...
            params = bof.parseCreateVocabularyInputs(varargin{:});
        ...
        %------------------------------------------------------------------
            ...
            featureLength = bof.checkCustomExtractorSizes(features, featureMetrics, varargout{:});
    ...
    methods (Hidden, Static, Access = protected)
        ...
        function [tf, ps] = checkAndAssignPointSelection(pointSelection)
            ...
            ps = validatestring(pointSelection,...
                bof.ValidPointSelectionOptions, mfilename, 'PointSelection');
        ...
        function featureLength = validateCustomExtractor(imgSet, extractor) 
            ...
            featureLength = bof.checkCustomExtractorSizes(features, metrics);
        ...
        function [imgSets, params] = parseInputs(varargin)                     
            ...
            parser = inputParser;
            parser.addRequired('imds', @(x)bof.checkImageSet(x,mfilename));
            
            parser.addParameter('VocabularySize',    d.VocabularySize,    @bof.checkVocabularySize);
            parser.addParameter('StrongestFeatures', d.StrongestFeatures, @bof.checkStrongestFeatures);
            parser.addParameter('PointSelection',    d.PointSelection,    @bof.checkAndAssignPointSelection);
            parser.addParameter('GridStep',          d.GridStep,          @bof.checkGridStep);
            parser.addParameter('BlockWidth',        d.BlockWidth,        @bof.checkBlockWidth);
            ...
            if bof.isImageSet(parser.Results.imds)                
            ...
            params.ExtractorOutputsLocations = bof.checkCustomExtractor(parser.Results.CustomExtractor);
            ...
            if wasSpecified('CustomExtractor')
                ...
                featureLength = bof.validateCustomExtractor(imgSets,params.CustomExtractor);
            ...
            [~, params.PointSelection] = ...
                bof.checkAndAssignPointSelection(parser.Results.PointSelection);
        ...
        function params = parseEncodeInputs(varargin)            
            ...
            defaults = bof.getDefaultSettings;
            ...
            bof.validateEncodeInput(parser.Results.imds);
        ...
        function printImageSetDescription(printer,imgSets)
            cats = bof.getCategoryStrings(imgSets);
    ...
    methods (Hidden, Static)
        ...
        function this = loadobj(s)
            ...
            if all(isfield(s,{'CustomExtractor', 'Upright', 'ExtractorOutputsLocations'})) % added in R2015a                                                                 
                ...
                bof.checkCustomExtractorOnLoad(s.CustomExtractor);                               
